...
const groups = computed<CurrencyGroup[]>(() => {
  ...
  return [...byCurrency.entries()].map(([currency, list]) => {
    const gross = Math.max(list.reduce((sum, a) => sum + Math.abs(a.balance), 0), 1);
    const net = list.reduce((sum, a) => sum + a.balance, 0);
    const sorted = [...list].sort((a, b) => Math.abs(b.balance) - Math.abs(a.balance));
    const positiveCount = sorted.filter(a => a.balance >= 0).length;
    const step = positiveCount > 1 ? 0.55 / (positiveCount - 1) : 0;

    let positiveIndex = 0;
    const segments = sorted.map<Segment>(a => {
      ...
      return {
        ...
        name: a.name,
        balance: a.balance,
        sharePct: Math.round((Math.abs(a.balance) / gross) * 100),
        negative,
        opacity,
      };
    });
